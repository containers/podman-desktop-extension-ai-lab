...
export class OpenVINO extends InferenceProvider {
  ...
  protected async getContainerCreateOptions(
    ...
    let cmd: string[] = ['--rest_port', '8000', '--config_path', `/model/${CONFIG_FILE_NAME}`, '--metrics_enable'];
    ...
    if (gpu) {
      ...
      switch (vmType) {
        case VMType.WSL:
          ...
          cmd = [
            '-c',
            '/usr/bin/ln -sfn /usr/lib/wsl/lib/* /usr/lib64/ && PATH="${PATH}:/usr/lib/wsl/lib/" && /ovms/bin/ovms --model_path /model --rest_port 8000',
          ];
          break;
        case VMType.LIBKRUN:
        ...
        case VMType.UNKNOWN:
          ...
          cmd = ['-c', '/ovms/bin/ovms --model_path /model --rest_port 8000'];
  ...
  async perform(config: InferenceServerConfig): Promise<InferenceServer> {
    ...
    const imageInfo: ImageInfo = await this.pullImage(
      connection,
      config.image ?? this.getOpenVINOInferenceImage(vmType, gpu),
      config.labels,
    );
  ...
  protected getOpenVINOInferenceImage(_vmType: VMType, _gpu?: IGPUInfo): string {
