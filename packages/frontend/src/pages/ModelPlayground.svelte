...
const getActionIcon = () => {
  if (playgroundState === undefined) return faWarning;
...
const onAction = () => {
  if (playgroundState === undefined || model?.id === undefined) return;
...
const isPromptable = () => {
  if (playgroundState === undefined) return false;
...
const isLoading = () => {
  if (playgroundState === undefined) return true;
...
const navigateToContainer = () => {
  if (playgroundState?.container?.containerId === undefined) return;
