...
import { ExtensionAILabPreferencesPage } from './model/preferences-extension-ai-lab-page';
  ...
  test.describe.serial('AI Lab API endpoint e2e test', { tag: '@smoke' }, () => {
    ...
    test.beforeAll(
      ...
      async ({ page, runner, navigationBar }) => {
        const extensions = await navigationBar.openExtensions();
        extensionVersion = await extensions.getInstalledExtensionVersion('ai-lab', AILabPage.AI_LAB_CATALOG_EXTENSION_LABEL);
